...
`0.3.1` — функциональный MVP: редактор, серверное хранение, привязка сущностей, мониторинг и опциональное управление нагрузкой.


### Изменения 0.3.0

- Надёжная анимация одной бегущей точки по активной линии; скорость зависит от мощности (максимум при 10 кВт+).
- Каталог узлов всегда доступен и включает инвертор, солнечные панели, аккумулятор и PE-заземление.
- Механические автоматы и УЗО могут работать без entity и наследуют нагрузку downstream.


### Исправление 0.3.1

- Версия backend-константы синхронизирована с manifest/frontend, чтобы URL модуля менялся при обновлении и Home Assistant не держал старый JS из кэша.
